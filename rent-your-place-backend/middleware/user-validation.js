module.exports = async (request, response, next) => {
    ...
    function isPasswordValid(password) {
        return /^(?=.*[0-9])(?=.*[A-Z])(?=.*[a-z])(?=.*[!@#$%^&*\-_.?]).{8,50}$/.test(password);
    }

    if (request.path === "/api/users/new") {
        const {userType, firstName, lastName, emailAddress, phoneNumber, password, confirmPassword} = request.body;
        if (![userType, firstName, lastName, emailAddress, phoneNumber, password, confirmPassword].every(Boolean))
            ...
            return response.status(422).json({message: "Confirm password does not match with password"});
        ...
        const {userType, firstName, lastName, phoneNumber, password, confirmPassword} = request.body;
