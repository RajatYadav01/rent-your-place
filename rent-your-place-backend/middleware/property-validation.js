module.exports = async (request, response, next) => {
    const {id, country, place, totalArea, numberOfBedrooms, numberOfBathrooms, nearbyLandmark, ownerID} = request.body;
    ...
    if (request.path === "/api/properties/new") {
        if (![country, place, totalArea, numberOfBedrooms, numberOfBathrooms, nearbyLandmark, ownerID].every(Boolean))
            ...
            return response.status(403).json({message: "Invalid number of bedrooms"});
        else if (!isTotalAreaValid(numberOfBathrooms))
            return response.status(403).json({message: "Invalid number of bathrooms"});
            ...
            return response.status(401).json({message: "Not authorized to add property"});
    } else if (request.path === "/api/properties/update" || request.path === "/api/properties/delete" || request.path === "/api/properties/get-owner") {
        if (![id, country, place, totalArea, numberOfBedrooms, numberOfBathrooms, nearbyLandmark, ownerID].every(Boolean))
            return response.status(403).json({message: "Missing details"});
