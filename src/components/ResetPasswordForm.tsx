...
const inputStatusReducer = (state: ResetPasswordFormInputStatusType, action: ResetPasswordFormInputActionType) => {
    switch(action.type) {
        case "focus":
...
const errorReducer = (state: ResetPasswordFormErrorsType, action: ResetPasswordFormErrorsActionType) => {
    switch(action.type) {
        case "emailAddress":
...
const passwordVisibilityStatusReducer = (state: ResetPasswordFormPasswordVisibilityStatusType, action: ResetPasswordFormPasswordVisibilityActionType) => {
    switch(action.name) {
        case "password":
...
function ResetPasswordForm(props: ResetPasswordFromPropsType) {
    const [resetPasswordFormState, setResetPasswordFormState] = useState<ResetPasswordFormType>({
        emailAddress: "",
        password: "",
        confirmPassword: ""
    });
    ...
    const [emailAddressInputStatus, dispatchEmailAddressInputStatus] = useReducer(inputStatusReducer, resetPasswordFormInputStatusInitialState);
    const [passwordInputStatus, dispatchPasswordInputStatus] = useReducer(inputStatusReducer, resetPasswordFormInputStatusInitialState);
    const [confirmPasswordInputStatus, dispatchConfirmPasswordInputStatus] = useReducer(inputStatusReducer, resetPasswordFormInputStatusInitialState);

    const [passwordVisibilityStatus, dispatchPasswordVisibilityStatus] = useReducer(passwordVisibilityStatusReducer, resetPasswordFormPasswordVisibilityStatusInitialState);
    ...
    const [errorMessage, dispatchErrorMessage] = useReducer(errorReducer, resetPasswordFormErrorsInitialState);    
    ...
    const isResetPasswordButtonDisabled = !emailAddressInputStatus.valid || !passwordInputStatus.valid || !confirmPasswordInputStatus.valid;
    ...
    useEffect(() => {
        const emailAddressValidationResult = emailAddressRegEx.test(resetPasswordFormState.emailAddress);
        dispatchEmailAddressInputStatus({type: "valid", payload: emailAddressValidationResult});
    }, [resetPasswordFormState.emailAddress]);
 
    useEffect(() => {
       const passwordValidationResult = passwordRegEx.test(resetPasswordFormState.password);
       dispatchPasswordInputStatus({type: "valid", payload: passwordValidationResult});
    }, [resetPasswordFormState.password]);

    useEffect(() => {
        const confirmPasswordValidationResult = (passwordRegEx.test(resetPasswordFormState.confirmPassword) && resetPasswordFormState.confirmPassword === resetPasswordFormState.password) ? true : false;
    ...
    const handleResetPassword = async (event: React.ChangeEvent<HTMLFormElement>) => {
        ...
        const confirmPasswordValidation = (() => { 
            ...
            } else if (!(passwordRegEx.test(resetPasswordFormState.confirmPassword) && resetPasswordFormState.confirmPassword === resetPasswordFormState.password)) {
                dispatchErrorMessage({type: "confirmPassword", payload: "Confirm password not matching with password."});
                dispatchConfirmPasswordInputStatus({type: "valid", payload: false});
            }
        })();
        if (!emailAddressInputStatus.valid || !passwordInputStatus.valid || !confirmPasswordInputStatus.valid) {
        ...
        if (emailAddressInputStatus.valid && passwordInputStatus.valid && confirmPasswordInputStatus.valid) {
            try {
                ...
                const serverResponse = await axios.patch("http://localhost:3050/api/users/resetPassword", JSON.stringify(resetPasswordFormState), 
                    {
                ...
                if (axios.isAxiosError(error)) { 
                    if (error) {
                        ...
                        dispatchErrorMessage({type: "resetPassword", payload: errorMessage});
                        setLoadingIconState(false);
                    }
                }
    ...
    return (
        <Fragment>
            {errorMessage.resetPasswordError && <p ref={resetPasswordErrorRef} className="reset-password-form__show-error" aria-live="assertive">{errorMessage.resetPasswordError}</p>}
            <form className="reset-password-form" onSubmit = {handleResetPassword} method="POST">
                <div className="reset-password-form__floating-label-group">
                    <input 
                        id="reset-password-form__email-address" 
                        className={`reset-password-form__email-address ${resetPasswordFormState.emailAddress !== "" ? (emailAddressInputStatus.valid ? "valid-input" : "invalid-input") : ""}`} 
                        lang="en" 
                        name="emailAddress"  
                        type="email" 
                        maxLength={150} 
                    ...
                    <label lang="en" className="reset-password-form__floating-label" htmlFor="reset-password-form__email-address">Enter your email address</label>
                </div>
                {errorMessage.emailAddressError && <p ref={emailAddressErrorRef} className="reset-password-form__show-error" aria-live="assertive">{errorMessage.emailAddressError}</p>}
                <div className="reset-password-form__floating-label-group">
                    <input 
                        id="reset-password-form__password" 
                        className={`reset-password-form__password ${resetPasswordFormState.password !== "" ? (passwordInputStatus.valid ? "valid-input" : "invalid-input") : ""}`} 
                        lang="en" 
                        name="password" 
                    ...
                    <label lang="en" className="reset-password-form__floating-label" htmlFor="reset-password-form__password">Enter new password</label>
                    <span className="reset-password-form__password-visibility-icon"><i className={`bi ${passwordVisibilityStatus.passwordVisibilityIcon}`} onClick={() => togglePasswordVisibilityStatus("password")}></i></span>
                </div>
                {errorMessage.passwordError && <p ref={passwordErrorRef} className="reset-password-form__show-error" aria-live="assertive">{errorMessage.passwordError}</p>}	
                <div className="reset-password-form__floating-label-group">
                    <input 
                        id="reset-password-form__confirm-password" 
                        className={`reset-password-form__confirm-password ${resetPasswordFormState.confirmPassword !== "" ? (confirmPasswordInputStatus.valid ? "valid-input" : "invalid-input") : ""}`} 
                        lang="en" 
                    ...
                    <label lang="en" className="reset-password-form__floating-label" htmlFor="reset-password-form__confirm-Password">Confirm password</label>
                    <span className="reset-password-form__password-visibility-icon"><i className={`bi ${passwordVisibilityStatus.confirmPasswordVisibilityIcon}`} onClick={() => togglePasswordVisibilityStatus("confirmPassword")}></i></span>
                </div>
                {errorMessage.confirmPasswordError && <p ref={confirmPasswordErrorRef} className="reset-password-form__show-error" aria-live="assertive">{errorMessage.confirmPasswordError}</p>}
                ...
                <button className={`reset-password-form__button--reset ${isResetPasswordButtonDisabled ? "button-disabled" : ""}`} lang="en" name="resetPassword" type="submit" disabled={isResetPasswordButtonDisabled ? true : false}><i></i>RESET</button>
            </form>
        </Fragment>
    );
}
