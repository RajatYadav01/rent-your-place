...
function Owner(props: OwnerPropsType) {    
    ...
    const [propertyFormInitialValues, setPropertyFormInitialValues] = useState<PropertyFormValuesType>(propertyFormValuesInitialState);
    ...
    const verifyUser = async (): Promise<void> => {
        try {
            ...
            if (axios.isAxiosError(err)) { 
                ...
                console.error(error);
                if (props.isLogOutTimerActive.current || localStorage.getItem("userType") === "owner" || localStorage.getItem("token")) {
    ...
    return (
        <div className="owner">
            <div className="owner__form-container">
                <Outlet context={{setNewPropertyAdded, propertyFormInitialValues, setPropertyUpdated, isLogOutTimerActive: props.isLogOutTimerActive, clearLogOutTimer: props.clearLogOutTimer}} />
            </div>
            {ownerError && <p className="owner__show-error" aria-live="assertive">{ownerError}</p>}
            <Properties userID={userID} newPropertyAdded={newPropertyAdded} propertyUpdated={propertyUpdated} setPropertyFormInitialValues={setPropertyFormInitialValues} />
        </div>
    );
}
