...
const inputStatusReducer = (state: UpdateProfileFormInputStatusType, action: UpdateProfileFormInputActionType) => {
    switch(action.type) {
        case "focus":
...
function errorReducer (state: UpdateProfileFormErrorsType, action: UpdateProfileFormErrorsActionType) {
    switch(action.type) {
        case "userType":
...
const passwordVisibilityStatusReducer = (state: UpdateProfileFormPasswordVisibilityStatusType, action: UpdateProfileFormPasswordVisibilityActionType) => {
    switch(action.name) {
        case "password":
...
function UpdateProfileForm() {
    const { clearLogOutTimer } = useOutletContext<{isLogOutTimerActive: React.MutableRefObject<boolean>, clearLogOutTimer: () => void}>();

    const loginSuccessStatusContext = useContext(LoginSuccessStatusContext);

    const getUserDetails = async () => {
        try {
            const response: AxiosResponse = await axios.get("http://localhost:3050/api/users/get", {
                headers: {
            ...
            if (axios.isAxiosError(error)) { 
                if (error) {
                    ...
                    dispatchErrorMessage({type: "updateProfile", payload: errorMessage});
                }
            }
            return null;
        }
    ...
    const [updateProfileFormState, setUpdateProfileFormState] = useState<UpdateProfileFormType>(updateProfileFormEmptyState);
    ...
    const [userTypeInputStatus, dispatchUserTypeInputStatus] = useReducer(inputStatusReducer, updateProfileFormInputStatusInitialState);
    const [firstNameInputStatus, dispatchFirstNameInputStatus] = useReducer(inputStatusReducer, updateProfileFormInputStatusInitialState);
    const [lastNameInputStatus, dispatchLastNameInputStatus] = useReducer(inputStatusReducer, updateProfileFormInputStatusInitialState);
    const [emailAddressInputStatus, dispatchEmailAddressInputStatus] = useReducer(inputStatusReducer, updateProfileFormInputStatusInitialState);
    const [phoneNumberInputStatus, dispatchPhoneNumberInputStatus] = useReducer(inputStatusReducer, updateProfileFormInputStatusInitialState);
    const [passwordInputStatus, dispatchPasswordInputStatus] = useReducer(inputStatusReducer, updateProfileFormInputStatusInitialState);
    const [confirmPasswordInputStatus, dispatchConfirmPasswordInputStatus] = useReducer(inputStatusReducer, updateProfileFormInputStatusInitialState);

    const [passwordVisibilityStatus, dispatchPasswordVisibilityStatus] = useReducer(passwordVisibilityStatusReducer, updateProfileFormPasswordVisibilityStatusInitialState);
    ...
    const [errorMessage, dispatchErrorMessage] = useReducer(errorReducer, updateProfileFormErrorsInitialState);
    ...
    const isUpdateButtonDisabled = !firstNameInputStatus.valid || !lastNameInputStatus.valid || !emailAddressInputStatus.valid || !phoneNumberInputStatus.valid;
    ...
    useEffect(() => {
        const userTypeValidationResult = userTypeRegEx.test(updateProfileFormState.userType);
        dispatchUserTypeInputStatus({type: "valid", payload: userTypeValidationResult});
    }, [updateProfileFormState.userType]);

    useEffect(() => {
        const firstNameValidationResult = nameRegEx.test(updateProfileFormState.firstName);
        dispatchFirstNameInputStatus({type: "valid", payload: firstNameValidationResult});
    }, [updateProfileFormState.firstName]);

    useEffect(() => {
        const lastNameValidationResult = nameRegEx.test(updateProfileFormState.lastName);
        dispatchLastNameInputStatus({type: "valid", payload: lastNameValidationResult});
    }, [updateProfileFormState.lastName]);
     
    useEffect(() => {
        const emailAddressValidationResult = emailAddressRegEx.test(updateProfileFormState.emailAddress);
        dispatchEmailAddressInputStatus({type: "valid", payload: emailAddressValidationResult});
    }, [updateProfileFormState.emailAddress]);
 
    useEffect(() => {
       const phoneNumberValidationResult = phoneNumberRegEx.test(updateProfileFormState.phoneNumber);
       dispatchPhoneNumberInputStatus({type: "valid", payload: phoneNumberValidationResult});
    }, [updateProfileFormState.phoneNumber]);

    useEffect(() => {
        const passwordValidationResult = passwordRegEx.test(updateProfileFormState.password);
        dispatchPasswordInputStatus({type: "valid", payload: passwordValidationResult});
    }, [updateProfileFormState.password]);

    useEffect(() => {
        const confirmPasswordValidationResult = (passwordRegEx.test(updateProfileFormState.confirmPassword) && updateProfileFormState.confirmPassword === updateProfileFormState.password) ? true : false;
    ...
    const deleteUser = async () => {
        try {
            ...
            const serverResponse = await axios.delete("http://localhost:3050/api/users/delete", {
                    headers: {
                        "Content-Type": "application/json",
                        "Token": localStorage.token,
    ...
    const handleProfileUpdate = async (event: React.ChangeEvent<HTMLFormElement>) => {
        ...
        const confirmPasswordValidation = (() => { 
            if (!(passwordRegEx.test(updateProfileFormState.confirmPassword) && updateProfileFormState.confirmPassword === updateProfileFormState.password)) {
        ...
        const isPasswordInputValid = updateProfileFormState.password ? passwordInputStatus.valid : true;
        const isConfirmPasswordInputValid = updateProfileFormState.confirmPassword ? confirmPasswordInputStatus.valid : true;
        if (!userTypeInputStatus.valid || !firstNameInputStatus.valid || !lastNameInputStatus.valid || !phoneNumberInputStatus.valid || !isPasswordInputValid || !isConfirmPasswordInputValid) {
        ...
        if (userTypeInputStatus.valid && firstNameInputStatus.valid && lastNameInputStatus.valid && phoneNumberInputStatus.valid && isPasswordInputValid && isConfirmPasswordInputValid) {
            try {
                ...
                const serverResponse = await axios.patch("http://localhost:3050/api/users/update", JSON.stringify(updateProfileFormState), 
                    {
                        headers: {"Content-Type": "application/json"},
                        withCredentials: true
                    }
                );
                const isUpdateSuccessful = serverResponse.data.status === "Update successful" ? true : false;
                if (isUpdateSuccessful && serverResponse.data.userTypeUpdated === true && serverResponse.data.newUserType !== localStorage.getItem("userType")) {
                    toast.success("Profile details updated successfully. Please log in as the new user type.");
                    clearLogOutTimer();
                    localStorage.removeItem("token");
                    localStorage.removeItem("userType");
                    localStorage.removeItem("showLikedProperties");
                    ...
                    navigate(`${"/"+serverResponse.data.newUserType}/login`, {replace: true});
                } else if (isUpdateSuccessful && serverResponse.data.userTypeUpdated === false) {
                    setLoadingIconState(false);
                    getUserDetails();
    ...
    return (
        <Fragment>
            <form className="update-profile-form" onSubmit={handleProfileUpdate} method="POST">
                <h3 className="update-profile-form__heading">Update profile details</h3>
                <svg className="update-profile-form__button--close-form" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 20 20" xmlSpace="preserve" onClick={closeProfileForm}>
                    <path d="M2,2 18,18 M18,2 2,18" stroke="#323232" strokeWidth={2}/>
                </svg>
                {errorMessage.updateProfileError && <p ref={updateProfileErrorRef} className="update-profile-form__show-error" aria-live="assertive">{errorMessage.updateProfileError}</p>}
                <div className="update-profile-form__label-group">
                    <label lang="en" className="update-profile-form__label" htmlFor="update-profile-form__user-type">Choose your new user type: </label>
                    <select 
                        id="update-profile-form__user-type" 
                        className={`update-profile-form__user-type ${updateProfileFormState.userType !== "" ? (userTypeInputStatus.valid ? "valid-input" : "invalid-input") : ""}`} 
                        lang="en" 
                        name="userType" 
                    ...
                    <p className="update-profile-form__user-type-warning">NOTE: If you change your user type from Owner to Tenant then all the properties posted by you as Owner will be deleted permanently.</p>
                </div>
                {updateProfileFormState.userType && userTypeInputStatus.focused && !userTypeInputStatus.valid && <p id="update-profile-form__user-type-requirements" className="update-profile-form__show-instructions">{parse(inputInstructions.userTypeInstructions)}</p>}
                {errorMessage.userTypeError && <p ref={userTypeErrorRef} className="update-profile-form__show-error" aria-live="assertive">{errorMessage.userTypeError}</p>}
                <div className="update-profile-form__name-group">
                    <div className="update-profile-form__name-floating-label-group">
                        <input 
                            ...
                            className={`update-profile-form__first-name ${updateProfileFormState.firstName !== "" ? (firstNameInputStatus.valid ? "valid-input" : "invalid-input") : ""}`} 
                            lang="en" 
                            name="firstName" 
                            type="text" 
                            maxLength={75} 
                        ...
                        <label lang="en" className="update-profile-form__name-floating-label" htmlFor="update-profile-form__first-name">Update your first name</label>
                    </div>
                    <div className="update-profile-form__name-floating-label-group">    
                        <input 
                            id="update-profile-form__last-name"
                            className={`update-profile-form__last-name ${updateProfileFormState.lastName !== "" ? (lastNameInputStatus.valid ? "valid-input" : "invalid-input") : ""}`} 
                            lang="en" 
                            name="lastName" 
                            type="text" 
                            maxLength={75} 
                        ...
                        <label lang="en" className="update-profile-form__name-floating-label" htmlFor="update-profile-form__last-name">Update your last name</label>
                    </div>
                </div>
                {(updateProfileFormState.firstName && firstNameInputStatus.focused && !firstNameInputStatus.valid) && <p id="update-profile-form__first-name-requirements" className="update-profile-form__show-instructions">{parse(inputInstructions.firstNameInstructions)}</p>}
                {(updateProfileFormState.lastName && lastNameInputStatus.focused && !lastNameInputStatus.valid) && <p id="update-profile-form__last-name-requirements" className="update-profile-form__show-instructions">{parse(inputInstructions.lastNameInstructions)}</p>}
                {errorMessage.firstNameError && <p ref={firstNameErrorRef} className="update-profile-form__show-error" aria-live="assertive">{errorMessage.firstNameError}</p>}
                {errorMessage.lastNameError && <p ref={lastNameErrorRef} className="update-profile-form__show-error" aria-live="assertive">{errorMessage.lastNameError}</p>}
                <div className="update-profile-form__floating-label-group">
                    <p className="update-profile-form__email-address">{updateProfileFormState.emailAddress}</p>
                </div>
                <div className="update-profile-form__floating-label-group">
                    <input 
                        id="update-profile-form__phone-number" 
                        className={`update-profile-form__phone-number ${updateProfileFormState.phoneNumber !== "" ? (phoneNumberInputStatus.valid ? "valid-input" : "invalid-input") : ""}`} 
                        lang="en" 
                        name="phoneNumber" 
                        type="text" 
                        maxLength={14} 
                    ...
                    <label lang="en" className="update-profile-form__floating-label" htmlFor="update-profile-form__phone-number">Update your phone number with country code</label>
                </div>
                {updateProfileFormState.phoneNumber && phoneNumberInputStatus.focused && !phoneNumberInputStatus.valid && <p id="update-profile-form__phone-number-requirements" className="update-profile-form__show-instructions">{parse(inputInstructions.phoneNumberInstructions)}</p>}
                {errorMessage.phoneNumberError && <p ref={phoneNumberErrorRef} className="update-profile-form__show-error" aria-live="assertive">{errorMessage.phoneNumberError}</p>}	
                <div className="update-profile-form__floating-label-group">
                    <input 
                        id="update-profile-form__password" 
                        className={`update-profile-form__password ${updateProfileFormState.password !== "" ? (passwordInputStatus.valid ? "valid-input" : "invalid-input") : ""}`} 
                        lang="en" 
                        name="password" 
                    ...
                    <label lang="en" className="update-profile-form__floating-label" htmlFor="update-profile-form__password">Enter new password to update your password</label>
                    <span className="update-profile-form__password-visibility-icon"><i className={`bi ${passwordVisibilityStatus.passwordVisibilityIcon}`} onClick={() => togglePasswordVisibilityStatus("password")}></i></span>
                </div>
                {updateProfileFormState.password && passwordInputStatus.focused && !passwordInputStatus.valid && <p id="update-profile-form__password-requirements" className="update-profile-form__show-instructions">{parse(inputInstructions.passwordInstructions)}</p>}
                {errorMessage.passwordError && <p ref={passwordErrorRef} className="update-profile-form__show-error" aria-live="assertive">{errorMessage.passwordError}</p>}
                <div className="update-profile-form__floating-label-group">
                    <input 
                        id="update-profile-form__confirm-password" 
                        className={`update-profile-form__confirm-password ${updateProfileFormState.confirmPassword !== "" ? (confirmPasswordInputStatus.valid ? "valid-input" : "invalid-input") : ""}`} 
                        lang="en" 
                    ...
                    <label lang="en" className="update-profile-form__floating-label" htmlFor="update-profile-form__confirm-password">Confirm new password</label>
                    <span className="update-profile-form__password-visibility-icon"><i className={`bi ${passwordVisibilityStatus.confirmPasswordVisibilityIcon}`} onClick={() => togglePasswordVisibilityStatus("confirmPassword")}></i></span>
                </div>
                {updateProfileFormState.confirmPassword && confirmPasswordInputStatus.focused && !confirmPasswordInputStatus.valid && <p id="update-profile-form__confirm-password-requirements" className="update-profile-form__show-instructions">{parse(inputInstructions.confirmPasswordInstructions)}</p>}
                {errorMessage.confirmPasswordError && <p ref={confirmPasswordErrorRef} className="update-profile-form__show-error" aria-live="assertive">{errorMessage.confirmPasswordError}</p>}
                ...
                <div className="update-profile-form__buttons-container">
                    <button className={`update-profile-form__button update-profile-form__button--update ${isUpdateButtonDisabled ? "button-disabled" : ""}`} lang="en" name="update" type="submit" disabled={isUpdateButtonDisabled ? true : false}><i></i>UPDATE</button>
                    <button className={"update-profile-form__button update-profile-form__button--delete"} lang="en" name="update" type="button" onClick={() => setDisplayDeleteDialogBox(true)}><i></i>DELETE ACCOUNT</button>	
                </div>
            </form>
            {displayDeleteDialogBox && (<Fragment>
                <div className="modal show" style={{ display: "block" }} tabIndex={-1} data-bs-backdrop="static" data-bs-keyboard="false" role="dialog" aria-modal="true">
                    <div className="modal-dialog modal-lg modal-dialog-centered">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h5 className="modal-title">Confirm deletion</h5>
                                <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close" onClick={() => setDisplayDeleteDialogBox(false)}></button>
                            </div>
                            <div className="modal-body">
                                Are you sure you want to permanently delete your account?
                            </div>
                            <div className="modal-footer">
                                <button className="btn btn-secondary" data-bs-dismiss="modal" onClick={() => setDisplayDeleteDialogBox(false)}>Cancel</button>
