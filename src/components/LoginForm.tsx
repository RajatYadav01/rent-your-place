...
const inputStatusReducer = (state: LoginFormInputStatusType, action: LoginFormInputActionType) => {
    switch(action.type) {
        case "focus":
...
const errorReducer = (state: LoginFormErrorsType, action: LoginFormErrorsActionType) => {
    switch(action.type) {
        case "emailAddress":
...
const passwordVisibilityStatusReducer = (state: LoginFormPasswordVisibilityStatusType, action: LoginFormPasswordVisibilityActionType) => {
    switch(action.name) {
        case "password":
            return {...state, visibilityType: action.type, visibilityIcon: action.icon};
        default:
            throw new Error();
    }
};
...
function LoginForm() {
    ...
    const [emailAddressInputStatus, dispatchEmailAddressInputStatus] = useReducer(inputStatusReducer, loginFormInputStatusInitialState);
    const [passwordInputStatus, dispatchPasswordInputStatus] = useReducer(inputStatusReducer, loginFormInputStatusInitialState);
    
    const [passwordVisibilityStatus, dispatchPasswordVisibilityStatus] = useReducer(passwordVisibilityStatusReducer, loginFormPasswordVisibilityStatusInitialState);

    const togglePasswordVisibilityStatus = (inputFieldName: string) => {
        if (inputFieldName === "password" && passwordVisibilityStatus.visibilityType === "password")
            dispatchPasswordVisibilityStatus({name: inputFieldName, type: "text", icon: "bi-eye-fill"});
        else if (inputFieldName === "password" && passwordVisibilityStatus.visibilityType === "text")
            dispatchPasswordVisibilityStatus({name: inputFieldName, type: "password", icon: "bi-eye-slash-fill"});
    }
    
    const [errorMessage, dispatchErrorMessage] = useReducer(errorReducer, loginFormErrorsInitialState);    
    ...
    const isLoginButtonDisabled = !emailAddressInputStatus.valid || !passwordInputStatus.valid;
    ...
    useEffect(() => {
        const emailAddressValidationResult = emailAddressRegEx.test(loginFormState.emailAddress);
        dispatchEmailAddressInputStatus({type: "valid", payload: emailAddressValidationResult});
    }, [loginFormState.emailAddress]);
 
    useEffect(() => {
       const passwordValidationResult = passwordRegEx.test(loginFormState.password);
    ...
    const handleLogin = async (event: React.ChangeEvent<HTMLFormElement>) => {
        ...
        if (emailAddressInputStatus.valid && passwordInputStatus.valid) {
            try {
                ...
                const serverResponse = await axios.post("http://localhost:3050/api/users/authenticate", JSON.stringify(loginFormState), 
                    {
                ...
                if (serverResponse.data.jwtToken) {
                    ...
                    localStorage.setItem("userType", serverResponse.data.userType.toLowerCase());
                    loginSuccessStatusContext.dispatchLoginStatusState({type: LoginStatusActionType.SET_LOGGED_IN_STATUS, payload: true});
                    loginSuccessStatusContext.dispatchLoginStatusState({type: LoginStatusActionType.SET_LOGIN_USERTYPE, payload: serverResponse.data.userType.toLowerCase()});
                    setLoadingIconState(false);
                    toast.success("Logged in successfully");
                    navigate(location.state?.path || `/${serverResponse.data.userType.toLowerCase()}`, {replace: true});
                }
            } catch (error: unknown) {
    ...
    return (
        <Fragment>
            {errorMessage.loginError && <p ref={loginErrorRef} className="login-form__show-error" aria-live="assertive">{errorMessage.loginError}</p>}
            <form className="login-form" onSubmit={handleLogin} method="POST">
                <div className="login-form__floating-label-group">
                    <input 
                        ...
                        className={`login-form__email-address ${loginFormState.emailAddress !== "" ? (emailAddressInputStatus.valid ? "valid-input" : "invalid-input") : ""}`} 
                        lang="en" 
                        name="emailAddress"  
                        type="email" 
                        maxLength={150} 
                    ...
                    <label lang="en" className="login-form__floating-label" htmlFor="login-form__email-address">Enter your email address</label>
                </div>
                {errorMessage.emailAddressError && <p ref={emailAddressErrorRef} className="login-form__show-error" aria-live="assertive">{errorMessage.emailAddressError}</p>}
                <div className="login-form__floating-label-group">
                    <input 
                        id="login-form__password" 
                        className={`login-form__password ${loginFormState.password !== "" ? (passwordInputStatus.valid ? "valid-input" : "invalid-input") : ""}`} 
                        lang="en" 
                        name="password" 
                    ...
                    <label lang="en" className="login-form__floating-label" htmlFor="login-form__password">Enter your password</label>
                    <span className="login-form__password-visibility-icon"><i className={`bi ${passwordVisibilityStatus.visibilityIcon}`} onClick={() => togglePasswordVisibilityStatus("password")}></i></span>
                    <Link to="../reset-password" className="login-form__forgot-password">Forgot password?</Link>
                </div>
                {errorMessage.passwordError && <p ref={passwordErrorRef} className="login-form__show-error" aria-live="assertive">{errorMessage.passwordError}</p>}
                ...
                <button className={`login-form__button--login ${isLoginButtonDisabled ? "button-disabled" : ""}`} lang="en" name="login" type="submit" disabled={isLoginButtonDisabled ? true : false}><i></i>LOG IN</button>
            </form>
        </Fragment>
    );
}
