...
function Pagination(props: PaginationPropsType) {
    ...
    const paginationRange = (totalNumberOfPages: number, currentPage: number, numberOfSiblings: number = 1) => {
        ...
        let rightSiblingIndex = Math.min(currentPage + numberOfSiblings, totalNumberOfPages);
        ...
        if (!showLeftEllipsis && showRightEllipsis) {
            ...
            let rightItemsRange = range(totalNumberOfPages - rightItemsCount + 1, totalNumberOfPages);
    ...
    let numberOfPages = paginationRange(props.totalNumberOfPages, props.currentPageNumber);

    return (
        <nav className="properties__pagination" aria-label="...">
          <ul className="pagination justify-content-center">
            { props.currentPageNumber === 1 ? "" : <li onClick={() => props.paginate("Previous", 0)} className="page-item">
              <a className="page-link" href="#properties-list">Previous</a>
            </li> }
            { numberOfPages.map((page) => (
                <li className={`page-item ${props.currentPageNumber === page ? "active" : ""}`} aria-current="page" key={page} onClick={() => props.paginate("Change", page)} >
                    <a className="page-link" href="#properties-list">{page}</a>
                </li>
            )) }
            { props.currentPageNumber === props.totalNumberOfPages ? "" : <li className="page-item" onClick={() => props.paginate("Next", 0)}>
                <a className="page-link" href="#properties-list">Next</a>
            </li> }
          </ul>
        </nav>
    );
